...
def run():
    ...
    if waarschuwing:
        st.warning("Buses unexpectedly teleport between locations:")
    ...
    if fouten:
        ...
        st.success('All OK! No SOC violations.')

    omloop_selectie = st.selectbox("Select a bus number to view SOC", list(soc_per_omloop.keys()))

    fig, ax = plt.subplots()
    ax.plot(range(len(soc_per_omloop[omloop_selectie])), soc_per_omloop[omloop_selectie], label=f'Theoretical SOC value - Bus {omloop_selectie}', linestyle='-', marker='o')
    ax.axhline(y=min_waarde_SOC, color='r', linestyle='--', label=f'Minimal SOC ({int(min_waarde_SOC)})')
    ax.axhline(y=max_verbruik, color='r', linestyle='--', label=f'Maximal SOC ({int(max_verbruik)})')
    ax.set_xlabel("Index")
    ax.set_ylabel("SOC value (kWh)")
    ax.set_title(f"Theoretical SOC value - Bus {omloop_selectie}")
    ...
    def controleer_oplaadtijd(df, min_oplaadtijd):
        ...
        for i, row in df.iterrows():
            if pd.notnull(row['energieverbruik']) and row['energieverbruik'] <= 0:
    ...
    # ✅ Verbeterde functie met weergave-filter
    def check_dienst_ritten_reistijd(df_planning, df_afstand):
        dienst_ritten = df_planning[df_planning['activiteit'] == 'dienst rit'].copy()
        ...
        for _, row in dienst_ritten.iterrows():
            ...
            if not match.empty:
                min_time = match.iloc[0]['min reistijd in min']
                max_time = match.iloc[0]['max reistijd in min']
                if not (min_time <= row['reistijd_min'] <= max_time):
                    invalid_rides.append(row.to_dict())

        df_invalid = pd.DataFrame(invalid_rides)
        return df_invalid

    invalid_rides_df = check_dienst_ritten_reistijd(df_planning, df_afstand)
    if not invalid_rides_df.empty:
        st.warning("The following service journeys are outside the time limits:")
        kolommen_weg = ['energieverbruik', 'starttijd datum', 'eindtijd datum',
                        'starttijd_jd', 'eindtijd_jd', 'starttijd_min', 'eindtijd_min', 'SOC']
        kolommen_voor_display = [col for col in invalid_rides_df.columns if col not in kolommen_weg]
        st.dataframe(invalid_rides_df[kolommen_voor_display])
    else:
